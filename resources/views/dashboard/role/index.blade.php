        ...
        <div class="card-header">
            <div class="d-flex align-items-center">
                ...
                <div class="btn-group" role="group">
                    <a href="{{ route('role.create') }}" class="btn btn-primary" role="button"><i
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check form-check-muted m-0">
                                            <label class="form-check-label">
                                                <input type="checkbox" id="{{ $role->id }}" class="form-check-input">
                                            </label>
                                        </div>
                                    </td>
                                    <td>{{ $role->name }}</td>
                                    <td>
                                        ...
                                        onclick="deleteButton( {{ $role->id }} )" role="button">Delete</a>
                                        @endcan
                                        @endif
                                    </td>
                                </tr>
...
<script>
    ...
    function deleteSelected(id) {
        swalConfirm(function () {
            var ids = getSelected();
            request('post', '{{ route('role.destroy-ajax') }}', ids);
            deleteRowSelected();
        })
    }
    ...
    function deleteButton(id) {
        swalConfirm(function () {
            var ids = getSelected();
            request('post', '{{ route('role.destroy-ajax') }}', {id});
            removeRowButton(id);
        })
    }

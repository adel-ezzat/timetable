        ...
        <div class="card-header">
            <div class="d-flex align-items-center">
                ...
                <div class="btn-group" role="group">
                    <a href="{{ route('admin.create') }}" class="btn btn-primary" role="button"><i
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check form-check-muted m-0">
                                            <label class="form-check-label">
                                                <input type="checkbox" id="{{ $admin->id }}" class="form-check-input">
                                            </label>
                                        </div>
                                    </td>
                                    <td>{{ $admin->name }}</td>
                                    ...
                                    <td>
                                        ...
                                        @if ($admin->id != 1)
                                        @can('Delete Managers')
                                        <a href="#" class="btn btn-outline-danger" onclick="deleteButton( {{ $admin->id }} )" role="button">Delete</a>
                                        @endcan
                                        @endif
                                    </td>
                                </tr>
...
<script>
    ...
    function deleteSelected(id) {
        swalConfirm(function() {
            var ids = getSelected();
                 request('post', '{{ route('admin.destroy-ajax') }}', ids);
                 deleteRowSelected();
        })
    }
      ...
      function deleteButton(id) {
        swalConfirm(function() {
            var ids = getSelected();
                 request('post', '{{ route('admin.destroy-ajax') }}', {id});
                 removeRowButton(id);
        })
    }
